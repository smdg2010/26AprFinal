...
/**
 * This class implements the IWSTransportAgent
...
public class WSTransportAgent implements IWSTransportAgent, Runnable 
{
	/**
	...
	*/
	...
	/**
	* object of IWSDSTWrapper
	*/
	public  WSDSTWrapper lookUp; 
	...
	*/
	void WSLookUp()
	{
	 ...
	 try {
			lookUp = (WSDSTWrapper)Naming.lookup("lookUp");
	...
	/**
	 *  Creates a new WSTransportAgent and start its thread.
	 *  
	 */
	...
	{  
	 final Logger log=new Logger();
	   try 
		{
		   WSTransportAgent oWSTA = new WSTransportAgent();
	          new Thread(oWSTA).start();
	          log.info("WS registry lookup");
	          oWSTA.WSLookUp();
	...
	/**
	 * This is the proxy that uses the backend protocol. Since this proxy implements IWSTransportAgent, clients can use it transparently.
	 * 
	 * @version 1.1.0
	 * @see IWSTransportAgent
	 */
	
	public static class WSTransportAgentProxy extends WSTransportAgent
    implements Serializable, ITransportAgent
	{
		// A constructor, throws IOException, ClassNotFoundException
		public WSTransportAgentProxy() throws IOException,
	...
	private IWSTransportAgent oIWSTA;
		
	/**
	* This constructor preserves the backend object passed as an argument.
	*/
	
	public WSTransportAgentProxy(IWSTransportAgent poWSTA) throws ClassNotFoundException, IOException
	{
            this.oIWSTA = poWSTA;
	...
	*/
	...
	private static final String MSG_PREFIX = "WSTransportAgentProxy message: ";
	private static final String ERR_PREFIX = "WSTransportAgentProxy error: ";
	...
	*/
	  ...
	  throws DMSException
	{
		try 
			{
				printOut("WSTA getDemand is runnning ..."); 
	...
	*/
	...
	/**
	* This method gets the WS transport Agent
  	* @return ITransportAgent
  	* @throws NotBoundException
	*/
	
	
	public ITransportAgent getWSTA() throws Exception, NotBoundException 
	{
        try 
		{
			
        	IWSTransportAgent backend = (IWSTransportAgent)Naming.lookup("oTA");
			log.info(" lookup successful");
            return new WSTransportAgentProxy(backend);
        } 
		catch (RemoteException e) 
		{
